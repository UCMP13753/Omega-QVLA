...
TAG="smoothquantNaive_W4A4"

# Stage 1: build 3 shorts in parallel, W4A4, svd_rank=0 (= no lowrank head, pure SQ+RTN)
say "Stage 1 — build 3 short packs ($TAG, block_size=0, svd_rank=0)"
SUITE=object  GPU=0 A_BITS=4 SVD_RANK=0 PACK_TAG="$TAG" \
  bash "$SCR/build_svdqNaive_generic.sh" > "$LOG/build_object.log"  2>&1 &
PID_O=$!
SUITE=spatial GPU=1 A_BITS=4 SVD_RANK=0 PACK_TAG="$TAG" \
  bash "$SCR/build_svdqNaive_generic.sh" > "$LOG/build_spatial.log" 2>&1 &
PID_S=$!
SUITE=goal    GPU=2 A_BITS=4 SVD_RANK=0 PACK_TAG="$TAG" \
...
SUITE=object  GPUS="0,1,2,3,4,5,6,7" PORT_BASE=9800 OFF=10 TRIALS=10 \
  LABEL="$TAG" EVAL_ABITS=4 \
  bash "$SCR/eval_svdqNaive_generic.sh" > "$LOG/eval_object.log" 2>&1 &
PID_O=$!
SUITE=spatial GPUS="0,1,2,3,4,5,6,7" PORT_BASE=9810 OFF=10 TRIALS=10 \
  LABEL="$TAG" EVAL_ABITS=4 \
  bash "$SCR/eval_svdqNaive_generic.sh" > "$LOG/eval_spatial.log" 2>&1 &
PID_S=$!
SUITE=goal    GPUS="0,1,2,3,4,5,6,7" PORT_BASE=9820 OFF=10 TRIALS=10 \
  LABEL="$TAG" EVAL_ABITS=4 \
...
# Summary
say ""
say "=== $TAG (block=0, svd_rank=0) shorts off=10 ==="
