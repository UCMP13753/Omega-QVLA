...
class Gr00tPolicy(BasePolicy):
    ...
    def warmup(self, num_steps: int = 2) -> None:
        ...
        When ``GR00T_CALIB_SUITE`` is set (e.g., ``libero_object``), language
        prompts are sampled from the suite's BDDL task descriptions instead of
        the literal token ``"warmup"``. This gives DuQuant calibrators a far
        more representative activation distribution covering every task's
        vocabulary, eliminating per-task calibration leakage where the first
        task's rollout dominates the running-max percentile.
        """
        if num_steps <= 0:
            return
        try:
            ...
            calib_prompts = self._load_libero_calib_prompts() or ["warmup"]
            language_keys = []
            if "language" in self._modality_config:
                language_keys = list(self._modality_config["language"].modality_keys)
                for full_key in language_keys:
                    obs[full_key] = [calib_prompts[0]]

            if not obs:
                return

            print(
                f"[Gr00tPolicy] warmup: num_steps={num_steps}, "
                f"calib_prompts={len(calib_prompts)} unique"
            )
    ...
    @staticmethod
    def _load_libero_calib_prompts() -> list:
        """Read LIBERO suite task descriptions from BDDL filenames.

        Returns empty list if ``GR00T_CALIB_SUITE`` is unset or BDDL files
        not found — caller falls back to ``"warmup"``.
        """
        import re
        suite = os.environ.get("GR00T_CALIB_SUITE", "").strip()
        libero_root = os.environ.get("LIBERO_ROOT", "").strip()
        if not suite or not libero_root:
            return []
        bddl_dir = Path(libero_root) / "libero" / "libero" / "bddl_files" / suite
        if not bddl_dir.is_dir():
            return []
        prompts = []
        # libero_10 filenames have a "{ROOM}_SCENE{N}_" prefix; strip it.
        scene_prefix_re = re.compile(r"^[A-Z_]+_SCENE\d+_")
        for p in sorted(bddl_dir.glob("*.bddl")):
            stem = p.stem
            stem = scene_prefix_re.sub("", stem)
